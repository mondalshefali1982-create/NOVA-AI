...
function parseWebsiteProject(raw) {
  ...
  // Step 6: Last resort — if text looks like HTML, treat it as the html field, and extract CSS/JS dynamically
  if (/<html|<!DOCTYPE/i.test(text)) {
    console.warn("[Website Parser] Raw response appears to be raw HTML. Parsing style/script blocks.");
    
    // Extract inline <style> block contents using regex
    let css = "";
    ...
    // Extract inline <script> block contents using regex
    let js = "";
    const scriptMatches = text.match(/<script[^>]*>([\s\S]*?)<\/script>/gi);
    if (scriptMatches) {
      js = scriptMatches.map(m => {
        if (/<script[^>]+src=/i.test(m)) return ""; // skip external scripts
        return m.replace(/<script[^>]*>|<\/script>/gi, "").trim();
      }).filter(Boolean).join("\n\n");
    }
    
    // Clean up inline styles and scripts from the html so they don't double-render
    let cleanedHtml = text
      .replace(/<style[^>]*>[\s\S]*?<\/style>/gi, "")
      .replace(/<script[^>]*>([\s\S]*?)<\/script>/gi, (m) => {
        if (/<script[^>]+src=/i.test(m)) return m; // keep external references
        return "";
      });

    return { html: cleanedHtml, css: css, js: js, pages: { index: cleanedHtml }, name: "NOVA Website" };
  }

  // Emergency fallback to never return 504/500 because of AI output
...
function normalizeApiProject(project, body = {}) {
  ...
  const rawHtml = decodeHtmlEntities(source.html || source.pages?.index || existing.html || existing.pages?.index || "");
