...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    // Task 5: Log normalized HTML start
    console.log("NORMALIZED HTML START", normalized.html.substring(0, 200));
...
function parseWebsiteProject(raw) {
  // Task 1 & 5: Log the raw AI response
  ...
  if (!result) {
    ...
    if (stripped !== text) {
      ...
      if (fromStripped && isValidProjectShape(fromStripped)) {
        ...
        result = unwrapProject(fromStripped);
      }
    }
  }

  // Step 3: Extract balanced JSON
  if (!result) {
    const extracted = extractBalancedJson(text);
    if (extracted) {
      console.log("[Website Parser] Extracted JSON substring");
      ...
      if (fromExtracted && isValidProjectShape(fromExtracted)) {
        ...
        result = unwrapProject(fromExtracted);
      }
    }
  }
  ...
  if (!result) {
    ...
    if (repairedDirect) {
      ...
      if (fromRepaired && isValidProjectShape(fromRepaired)) {
        ...
        result = unwrapProject(fromRepaired);
      }
    }
  }
  ...
  if (!result) {
    ...
    if (extracted_fields.html || extracted_fields.css) {
      console.log("[Website Parser] SUCCESS: Recovered fields via regex");
      result = extracted_fields;
    }
  }

  // Step 6: Raw HTML parser (Case C)
  ...
  // Step 7: Emergency fallback if still nothing
  if (!result) {
  ...
  if (result && typeof result.html === "string") {
    ...
    if (!isValidHtml) {
      console.error("[Website Parser] VALIDATION ERROR: project.html does not contain valid HTML! Raw AI response was:", text);
      throw new Error("AI provider returned invalid HTML structure. Please retry generation.");
    }
  }

  // Task 5: Log parsed project
...
function tryParseJson(value) {
  try {
    ...
    // Task 2: Double parse if the result is still a JSON string
...
function normalizeApiProject(project, body = {}) {
  ...
  // Task 4: Verify typeof rawHtml === "string" and rawHtml.startsWith("<")
