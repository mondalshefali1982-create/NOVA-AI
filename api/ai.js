...
`;
      ...
      case "video": {
        ...
        // Model → fal.ai provider ID mapping
        const MODEL_TO_FAL = {
        ...
        const falModelId = MODEL_TO_FAL[model] || MODEL_TO_FAL[DEFAULT_MODEL];
        const queueBaseUrl = `https://queue.fal.run/${falModelId}`;

        // Build auth header: FAL_KEY uses "Key <key>", HF token uses "Bearer <key>"
        let authHeader;
        let useHfRouter = false;
        if (falKey) {
          authHeader = `Key ${falKey}`;
        } else {
          // Use HF router to proxy to fal-ai
          authHeader = `Bearer ${hfKey}`;
          useHfRouter = true;
        }

        // If using HF router, use the HF router URL instead of direct fal.ai
        const submitUrl = useHfRouter
          ? `https://router.huggingface.co/fal-ai/${falModelId.replace("fal-ai/", "")}`
          : queueBaseUrl;

        console.log(`[Video API] Prompt: "${prompt}" | Model: ${model} | FalModel: ${falModelId}`);
        console.log(`[Video API] AspectRatio: ${aspectRatio} | Duration: ${duration} | Quality: ${quality}`);
        console.log(`[Video API] Submit URL: ${submitUrl} | Auth: ${useHfRouter ? "HF Router" : "FAL Direct"}`);
        ...
        try {
          ...
          console.log(`[Video API] Submitting to queue...`);
          ...
          console.log(`[Video API] Submit response status: ${submitResponse.status}`);
          ...
          console.log(`[Video API] Submit response:`, JSON.stringify(submitData).slice(0, 500));
          ...
          if (submitData.video?.url || submitData.output?.video?.url) {
            ...
            console.log(`[Video API] Synchronous result received. Video URL: ${videoUrl.slice(0, 100)}...`);
