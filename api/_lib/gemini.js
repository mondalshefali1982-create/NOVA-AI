  ...
  if (googleApiKey) {
    try {
      ...
      const timeoutPromise = new Promise((_, reject) =>
        setTimeout(() => reject(new Error("Gemini primary request timed out after 25 seconds")), 25000)
      );
...
async function fetchWithAbortTimeout(url, options, timeoutMs, label = "request") {
