...
async function callWebsiteBackend(payload, options = {}) {
  const maxAttempts = options.attempts || 3;
  let lastError = null;

  console.log("[NOVA Website] callWebsiteBackend:", { action: payload.action, type: payload.type, pageMode: payload.pageMode, promptLength: String(payload.prompt || "").length });

  for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
    const controller = new AbortController();
    // FIX: Raised frontend timeout to 180s to match backend timeout
    ...
    try {
      ...
      console.log(`[NOVA Website] Attempt ${attempt}/${maxAttempts} — fetching ${NOVA_API_ROUTES.website}`);
      const response = await fetch(`${NOVA_BACKEND_BASE_URL}${NOVA_API_ROUTES.website}`, {
        method: "POST",
        headers,
        body: JSON.stringify(payload),
      ...
      console.log(`[NOVA Website] Response status: ${response.status}`, { hasHtml: Boolean(data?.html), hasCss: Boolean(data?.css), hasPages: Boolean(data?.pages) });

      if (!response.ok) {
        console.error(`[NOVA Website] Backend error ${response.status}:`, data?.error);
        const error = new Error(data.error || getWebsiteHttpError(response.status));
        error.status = response.status;
        throw error;
      }

      console.log("[NOVA Website] Success! Response keys:", Object.keys(data));
      return data;
    } catch (error) {
      lastError = error;
      console.error(`[NOVA Website] Attempt ${attempt} failed:`, error.message);
      if (attempt < maxAttempts) {
        setWebsiteStatus(getWebsiteRetryMessage(error), true);
        await waitForWebsiteRetry(900 * attempt);
      }
    } finally {
  ...
  throw new Error(getWebsiteRetryMessage(lastError, false));
}

function getWebsiteHttpError(status) {
  if (status === 504) return "Generation timeout. Please try again.";
  if (status === 502) return "AI provider returned invalid response. Please retry generation.";
  if (status === 503) return "AI providers are busy. Please try again shortly.";
  if (status === 400) return "Please check the website request and try again.";
  return "The website generator is temporarily unavailable.";
}

function getWebsiteRetryMessage(error, retrying = true) {
  const message = String(error?.message || "");
  const suffix = retrying ? " Retrying..." : "";
  if (error?.name === "AbortError" || /timeout/i.test(message)) return `Generation timeout.${suffix}`;
  if (/invalid|JSON|empty/i.test(message)) return `AI provider returned invalid response.${suffix}`;
  if (/fetch|network|connection|Failed to fetch/i.test(message)) return `Connection issue detected.${suffix}`;
  if (/busy|temporarily|unavailable|503|429/i.test(message)) return `AI providers are busy.${suffix}`;
  return message || `Website generation failed.${suffix}`;
}

function waitForWebsiteRetry(ms) {
...
async function generateWebsiteProject() {
  ...
  console.log("[NOVA Website] generateWebsiteProject called:", { prompt: prompt.slice(0, 80), type, pageMode });

  // FIX: Clear preview frame immediately and show loading state
  // This prevents stale content from old websites remaining visible
...
<body>
  ...
  <p>🚀 NOVA is generating your website...</p>
</body>
</html>`;
  }
  ...
  try {
    ...
    console.log("[NOVA Website] Backend response received. Normalizing project...");
    ...
    console.log("[NOVA Website] Project normalized:", {
      id: project.id,
      name: project.name,
      htmlLength: project.html?.length,
      cssLength: project.css?.length,
      jsLength: project.js?.length,
      pageKeys: Object.keys(project.pages || {})
    });

    // FIX: Deduplicate — remove any existing project with same prompt+type to avoid stale state
    state.websites = state.websites.filter(w => !(w.prompt === prompt && w.type === type && w.pageMode === pageMode));
    ...
    console.log("[NOVA Website] Generation complete.");
  } catch (error) {
    console.error("[NOVA Website] generateWebsiteProject FAILED:", error.message);
    const errorMessage = error.message || "NOVA could not generate the website right now.";
    setWebsiteStatus(errorMessage);

    // FIX: Show meaningful error in preview iframe instead of stale content
    if (previewFrame) {
      previewFrame.srcdoc = `<!DOCTYPE html>
<html>
<head><style>
...
<body>
  ...
  <p style="margin-top:1rem; opacity:0.5;">Please try again or adjust your prompt.</p>
</body>
</html>`;
    }
  } finally {
...
function renderWebsitePreview(project) {
  ...
  if (!project) {
    // FIX: Show empty state placeholder instead of leaving stale content
    frame.srcdoc = `<!DOCTYPE html>
<html>
<head><style>
  body { display:flex; flex-direction:column; justify-content:center; align-items:center; height:100vh;
         background:#0a0a14; color:#a0a0d0; font-family:system-ui,sans-serif; margin:0; text-align:center; }
...
<body>
  ...
  let html = project.pages?.[page] || project.pages?.index || project.html || "";

  console.log("[NOVA Website] renderWebsitePreview:", { page, htmlLength: html.length, hasCss: Boolean(project.css), hasJs: Boolean(project.js) });
...
<head><style>
  body { display:flex; flex-direction:column; justify-content:center; align-items:center; height:100vh;
         background:#0a0a14; color:#f87171; font-family:system-ui,sans-serif; margin:0; text-align:center; }
  p { font-size:0.9rem; }
</style></head>
<body><p>⚠️ No HTML content to preview for this page.</p></body>
</html>`;
    return;
  }

  // Ensure HTML, CSS, and JS are fully unescaped before rendering
  const unescapedProject = {
    ...project,
    html: unescapeHtml(html),
    css: unescapeHtml(project.css || ""),
    js: unescapeHtml(project.js || "")
  };

  frame.srcdoc = buildWebsitePreviewDocument(unescapedProject);
}
...
function buildWebsitePreviewDocument(project) {
  ...
  // Remove duplicate stylesheet and script tags that reference style.css and script.js
  // (because we inject them inline below)
  html = html
...
function getWebsiteFiles(project) {
  if (!project) return [];
  const files = [{ name: "index.html", content: prepareWebsiteHtmlForExport(project.pages?.index || project.html) }];
  if (project.pages?.about) files.push({ name: "about.html", content: prepareWebsiteHtmlForExport(project.pages.about) });
  if (project.pages?.services) files.push({ name: "services.html", content: prepareWebsiteHtmlForExport(project.pages.services) });
  if (project.pages?.contact) files.push({ name: "contact.html", content: prepareWebsiteHtmlForExport(project.pages.contact) });
  files.push({ name: "style.css", content: project.css || "" });
  files.push({ name: "script.js", content: project.js || "" });
  Object.entries(project.assets || {}).forEach(([name, content]) => {
    files.push({ name: name.startsWith("assets/") ? name : `assets/${name}`, content: String(content || "") });
  });
  if ((project.pageMode === "multi" || project.pages?.about || project.pages?.services || project.pages?.contact) && !files.some((file) => file.name.startsWith("assets/"))) {
    files.push({ name: "assets/README.txt", content: "NOVA AI generated project assets folder. Add local images here when replacing remote image URLs." });
  }
  return files;
}

function prepareWebsiteHtmlForExport(html = "") {
