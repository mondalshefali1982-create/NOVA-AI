...
function generateImage() {
  ...
  const url = `https://image.pollinations.ai/prompt/${encodeURIComponent(fullPrompt)}?width=768&height=512&nologo=true`;
...
function generatePlan() {
  ...
  state.planner = blocks.map(([time, title, text]) => ({ id: crypto.randomUUID(), time, title, text }));
