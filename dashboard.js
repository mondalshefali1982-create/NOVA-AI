...
async function generateWebsite(regenerateNote = "") {
  ...
  const prompt = websitePrompt.value.trim();
  const pageMode = document.querySelector('input[name="websitePageMode"]:checked')?.value || "single";
  ...
  try {
    const website = await callNovaBackend(NOVA_API_ROUTES.website, { prompt, regenerateNote, pageMode });
    const record = createWebsiteRecord(website, prompt);
    upsertWebsiteRecord(record);
    updateWebsiteProgress(100);
    renderWebsiteResult();
    renderWebsiteHistory();
    setWebsiteStatus("Preview launched. Website saved to history.", false);
    state.requestCount += 1;
    localStorage.setItem("novaRequestCount", String(state.requestCount));
    updateCounters();
  } catch (error) {
...
function normalizeClientWebsite(website) {
  ...
  return {
    ...
    files: {
      ...
      ...Object.fromEntries(Object.entries(files).filter(([name]) => name.endsWith(".html") && name !== "index.html"))
    }
  };
}

function createWebsiteRecord(website, prompt) {
  const normalized = normalizeClientWebsite(website);
  const now = Date.now();
  return normalizeWebsiteRecord({
    id: crypto.randomUUID(),
    name: normalized.meta.name || inferWebsiteName(prompt, normalized.meta.websiteType),
    prompt,
...
function renderWebsiteResult() {
  ...
  if (websiteProjectInfo) {
    websiteProjectInfo.innerHTML = `
      ...
      <span>Theme: ${escapeHtml(meta.colorTheme || "Auto")}</span>
      <span>Audience: ${escapeHtml(meta.targetAudience || "Auto")}</span>
    `;
  }
...
function renderWebsitePreview() {
  ...
  const files = website.files;
  const activeHtml = state.activeWebsiteCodeTab?.endsWith(".html") ? state.activeWebsiteCodeTab : "index.html";
  const html = (files[activeHtml] || files["index.html"] || "")
    .replace(/<link[^>]+href=["']style\.css["'][^>]*>/i, `<style>${files["style.css"]}</style>`)
    .replace(/<script[^>]+src=["']script\.js["'][^>]*><\/script>/i, `<script>${files["script.js"]}<\/script>`);
  websitePreviewFrame.srcdoc = html;
}
...
function setWebsiteCodeTab(tab = "index.html") {
  ...
  const website = getActiveWebsite();
  renderCodeTabButtons(website);
  document.querySelectorAll("[data-code-tab]").forEach((button) => {
    button.classList.toggle("active", button.dataset.codeTab === tab);
  });
  if (websiteCodeViewer) {
    websiteCodeViewer.textContent = website?.files?.[tab] || "Generated code will appear here.";
  }
  if (tab.endsWith(".html")) renderWebsitePreview();
}

function downloadWebsiteZip() {
  const website = getActiveWebsite();
  if (!website) return;
  const zipBlob = createZip({ ...website.files, "assets/.gitkeep": "" });
...
function normalizeWebsiteRecord(website) {
  ...
  return {
    ...
    thumbnail: website.thumbnail || extractFirstImage(files),
    meta: website.meta || {},
    files: {
      "index.html": files["index.html"] || "",
      "style.css": files["style.css"] || "",
...
function renderCodeTabButtons(website) {
  const tabs = document.querySelector(".website-tabs");
  if (!tabs || !website) return;
  const files = Object.keys(website.files || {});
  const htmlFiles = files.filter((name) => name.endsWith(".html"));
  const tabFiles = [...htmlFiles, "style.css", "script.js"].filter((name) => website.files[name] !== undefined);
  tabs.innerHTML = "";
...
async function loadCloudWebsites() {
  if (!state.authToken) return;
  try {
    const data = await callNovaAuth(NOVA_API_ROUTES.websitesSync, { method: "GET" });
...
async function syncAllWebsitesToCloud() {
  ...
  try {
    for (const website of state.websites) {
      await callNovaAuth(NOVA_API_ROUTES.websitesSync, {
        method: "POST",
        body: JSON.stringify({ action: "upsert", website: normalizeWebsiteRecord(website) })
      });
    }
  } catch (error) {
    setWebsiteStatus(`Website cloud sync paused: ${error.message}`, false);
