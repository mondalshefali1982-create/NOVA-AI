...
function normalizeWebsiteProject(response, prompt, type, pageMode = "single") {
  ...
  // Task 4: Verify typeof html === "string" and html.startsWith("<")
  if (typeof html !== "string" || !html.startsWith("<")) {
    console.error("[NOVA Website] normalizeWebsiteProject validation failed: html is not a valid HTML string starting with <. Got:", html);
    throw new Error("Normalized website HTML is invalid.");
  }
...
<body><p>⚠️ No HTML content to preview for this page.</p></body>
  ...
  frame.srcdoc = buildWebsitePreviewDocument({ ...project, html: decodeHtmlEntities(html) });
}
