...
async function generateWebsite(regenerateNote = "") {
  ...
  try {
    const website = await callNovaBackend(NOVA_API_ROUTES.website, {
      prompt,
      regenerateNote,
...
async function startVideoGeneration() {
  ...
  try {
    ...
    const response = await callNovaBackend(NOVA_API_ROUTES.video, payload);
...
async function loadCloudVideos() {
  if (!state.authToken) return;
  try {
    const data = await callNovaAuth(NOVA_API_ROUTES.videosSync, { method: "GET" });
    if (data.videos) {
...
async function syncAllVideosToCloud() {
  ...
  try {
    ...
    for (const video of state.videos) {
      await callNovaAuth(NOVA_API_ROUTES.videosSync, {
        method: "POST",
        body: JSON.stringify({ action: "upsert", video })
      });
    }
...
async function deleteCloudVideo(id) {
  return callNovaAuth(NOVA_API_ROUTES.videosSync, {
    method: "POST",
    body: JSON.stringify({ action: "delete", id })
  });
}
