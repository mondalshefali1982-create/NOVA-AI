...
const state = {
  ...
  chatHistory: store.get("novaChatHistory", []),
...
function restoreChat() {
  if (state.chatHistory.length) {
    state.chatHistory.slice(-10).forEach((message) => addMessage(chatMessages, message.type, message.text));
    state.chatHistory.slice(-2).forEach((message) => addMessage(overviewMessages, message.type, message.text));
    return;
  }

  const welcome = "Welcome to NOVA. Ask me to plan a launch, summarize a task list, generate a prompt, or sharpen an idea.";
  addMessage(chatMessages, "ai", welcome);
  addMessage(overviewMessages, "ai", "Workspace ready. Your AI command center is online.");
  saveChatMessage("ai", welcome);
}
...
function saveChatMessage(type, text) {
  state.chatHistory.push({ type, text, createdAt: Date.now() });
  state.chatHistory = state.chatHistory.slice(-30);
  store.set("novaChatHistory", state.chatHistory);
}
