...
async function generateWebsiteProject() {
  ...
  try {
    const response = await callNovaBackend(NOVA_API_ROUTES.website, { prompt, type });
    const project = normalizeWebsiteProject(response, prompt, type);
...
function normalizeWebsiteProject(response, prompt, type) {
...
function renderWebsiteHistory() {
  ...
  state.websites.forEach((project) => {
    ...
    button.innerHTML = `
      <strong>${escapeHtml(project.type)} � ${escapeHtml(project.prompt)}</strong>
      <span>${formatConversationTime(project.createdAt)}</span>
    `;
    button.addEventListener("click", () => {
      state.activeWebsiteId = project.id;
...
function renderWebsiteCode(project) {
  ...
  if (htmlCode) htmlCode.textContent = project?.html || "Generated index.html will appear here.";
...
function renderWebsitePreview(project) {
  ...
  frame.srcdoc = buildWebsitePreviewDocument(project);
}
...
function buildWebsitePreviewDocument(project) {
  ...
  if (/<\/head>/i.test(html)) {
    return html.replace(/<\/head>/i, `${css}</head>`).replace(/<\/body>/i, `${js}</body>`);
  }

  return `<!DOCTYPE html><html><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0">${css}</head><body>${html}${js}</body></html>`;
}
...
function activateWebsiteTab(tab) {
  ...
  Object.entries(panels).forEach(([key, element]) => {
    element?.classList.toggle("active", key === state.activeWebsiteTab);
  });
}
...
function getWebsiteFileContent(project, type) {
  if (!project) return "";
  if (type === "html") return project.html;
