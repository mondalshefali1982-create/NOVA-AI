...
function enhanceWebsitePrompt() {
  ...
  if (!value) {
    websitePrompt.value = "Create a modern portfolio website for a UI/UX designer with a polished homepage, case studies, services, testimonials, contact section, dark responsive design, accessible navigation, and conversion-focused copy.";
  } else if (!value.includes("Include")) {
    websitePrompt.value = `${value}\n\nInclude a complete homepage, semantic sections, SEO basics, accessible navigation, responsive desktop/tablet/mobile layouts, polished copy, modern typography, strong calls to action, and separate HTML, CSS, and JavaScript files.`;
  }
  updateWebsitePromptCount();
}
...
async function generateWebsite(regenerateNote = "") {
  ...
  const steps = ["Planning Website...", "Generating Layout...", "Writing HTML...", "Creating CSS...", "Adding JavaScript...", "Optimizing..."];
  let stepIndex = 0;
  ...
  try {
    const website = await callNovaBackend(NOVA_API_ROUTES.website, { prompt, regenerateNote });
    state.currentWebsite = normalizeClientWebsite(website);
    state.currentWebsitePrompt = prompt;
    renderWebsiteResult();
    setWebsiteStatus("Done.", false);
    state.requestCount += 1;
    localStorage.setItem("novaRequestCount", String(state.requestCount));
    updateCounters();
  } catch (error) {
...
function renderWebsiteResult() {
  const website = state.currentWebsite;
  if (!website) return;
...
function renderWebsitePreview() {
  if (!websitePreviewFrame || !state.currentWebsite) return;
  const files = state.currentWebsite.files;
...
function setWebsiteCodeTab(tab = "index.html") {
  ...
  if (websiteCodeViewer) {
    websiteCodeViewer.textContent = state.currentWebsite?.files?.[tab] || "Generated code will appear here.";
  }
}

function downloadWebsiteZip() {
  if (!state.currentWebsite) return;
  const zipBlob = createZip({ ...state.currentWebsite.files, "assets/.gitkeep": "" });
  const link = document.createElement("a");
  link.href = URL.createObjectURL(zipBlob);
  link.download = `nova-ai-website-${Date.now()}.zip`;
