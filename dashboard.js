...
async function loginUser() {
  ...
  try {
    const data = await callNovaAuth(NOVA_API_ROUTES.auth, { method: "POST", body: JSON.stringify({ action: "login", email, password }) });
...
async function signupUser() {
  ...
  try {
    const data = await callNovaAuth(NOVA_API_ROUTES.auth, { method: "POST", body: JSON.stringify({ action: "signup", name, email, password }) });
    completeAuthSession(data);
    await loadCloudConversations();
    await loadCloudWebsites();
    await loadCloudVideos();
...
async function askFreeAI(message) {
  const response = await callNovaBackend(NOVA_API_ROUTES.ai, { action: "chat", message:
...
async function generateWebsite(regenerateNote = "") {
  ...
  try {
    const website = await callNovaBackend(NOVA_API_ROUTES.ai, { action: "website",
      prompt,
      regenerateNote,
...
async function syncAllWebsitesToCloud() {
  ...
  try {
    for (const website of state.websites) {
      await callNovaAuth(NOVA_API_ROUTES.sync, { method: "POST", body: JSON.stringify({ action: "website", subAction: "upsert", website: normalizeWebsiteRecord(website) }) });
    }
  } catch (error) {
    setWebsiteStatus(`Website cloud sync paused: ${error.message}`, false);
  } finally {
...
async function deleteCloudWebsite(id) {
  if (!state.authToken || !id) return;
  try {
    await callNovaAuth(NOVA_API_ROUTES.sync, { method: "POST", body: JSON.stringify({ action: "website", subAction: "delete", id }) });
  } catch (error) {
...
async function callNovaImageBackend(payload) {
  ...
  const response = await fetch(`${NOVA_BACKEND_BASE_URL}${NOVA_API_ROUTES.ai}`, { method: "POST", headers, body: JSON.stringify({ action: "image", ...payload }) });
...
function renderVideoHistory() {
  ...
  let filtered = state.videos.filter(v => {
    return v.prompt.toLowerCase().includes(searchQuery) || (v.name && v.name.toLowerCase().includes(searchQuery));
  });
      ...
      <div class="video-history-item ${isActive}" data-id="${video.id}">
        ...
        <div class="history-details">
          ...
          <div class="history-meta">
            <span>${video.model.split("/").pop()}</span>
...
async function syncAllVideosToCloud() {
  ...
  try {
    ...
    for (const video of state.videos) {
      await callNovaAuth(NOVA_API_ROUTES.sync, { method: "POST", body: JSON.stringify({ action: "video", subAction: "upsert", video }) });
    }
...
async function deleteCloudVideo(id) {
  return callNovaAuth(NOVA_API_ROUTES.sync, { method: "POST", body: JSON.stringify({ action: "video", subAction: "delete", id }) });
}
